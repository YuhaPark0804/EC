...
void TIM_init(TIM_TypeDef* TIMx){     
	...
	else RCC->APB2ENR |= RCC_APB2ENR_TIM11EN;
    ...
    // 3. CNT Direction
	TIMx->CR1 &= ~(TIM_CR1_DIR);					// Upcounter, ~(1<<4)
...
void TIM_period_us(TIM_TypeDef *TIMx, uint32_t usec){   
	...
	if (TIMx == TIM2 || TIMx == TIM5){
		...
		PSCval = Sys_CLK / 1e+06;						            // 84 or 16	--> f_cnt = 1MHz
		ARRval = Sys_CLK/PSCval/1e+06 * usec;						// ARRval= 1*usec, 84M/84/1e+06 = 1
																										//1 *20000 (ARR = reload value)
		TIMx->PSC = PSCval - 1;
		TIMx->ARR = ARRval - 1;				
	}
	else{
		...
		PSCval = Sys_CLK / 1e+06;							            // 84 or 16	--> f_cnt = 1MHz
		ARRval = Sys_CLK/PSCval/1e+06 * usec;						// ARRval= 1*usec
		TIMx->PSC = PSCval - 1;                           // 84 - 1
		TIMx->ARR = ARRval - 1;                           // 1*usec - 1
	}			
}
...
void TIM_period_ms(TIM_TypeDef* TIMx, uint32_t msec){ 
	...
	if (TIMx == TIM2 || TIMx == TIM5){
		uint32_t ARRval;		
		PSCval = Sys_CLK/1e+06;		                // 84 or 16	--> PSC_clk=f_cnt = 1MHz
		ARRval = Sys_CLK/PSCval/1000 * msec;			// 1MHz*msec,  ARRval=1000 for 1msec, 84M/84/1000 = 1000
																							
		TIMx->PSC = PSCval - 1;                   //84 - 1
		TIMx->ARR = ARRval - 1;                   //100 *msec - 1
	}
	else{
		uint16_t ARRval;

		PSCval = Sys_CLK/1e+04;									//8400
		ARRval = Sys_CLK/PSCval/1000 * msec;		//100kHz*msec,  ARRval=10 for 1msec, 84M/8400/1000 = 10
		TIMx->PSC = PSCval - 1;
		TIMx->ARR = ARRval - 1;
	}
}
...
void TIM_UI_init(TIM_TypeDef* TIMx){
    // 1. Initialize Timer	
	TIM_init(TIMx);
