...
* @brief   Embedded Controller:  EC_HAL_for_student_exercise. LAB_PWM
* 
******************************************************************************
*/
...
// 0. Match TIMx from  Port and Pin 	
	...
	GPIO_otype(pinName, PUSH_PULL); 
	GPIO_pupd(pinName, NO_PUPD); 					
...
// 2. Configure GPIO AFR by Pin num.	
	...
	/*
	mux selecting an aternate function
	p.150 (en.DM)
	*/

	uint32_t num = 0;
	
	if(TIMx == TIM1 || TIMx == TIM2) num = 0x01;
	else if(TIMx == TIM3 || TIMx == TIM4 || TIMx == TIM5) num = 0x02;
	else if(TIMx == TIM9 || TIMx == TIM10 || TIMx == TIM11) num = 0x03;
	
	port->AFR[num/8] |= num<<(4*(pin%8));

	
// 3. Initialize Timer 
	TIM_init(TIMx);	         													    // with default msec=1msec value.		
	TIMx->CR1 &= ~TIM_CR1_CEN;   													// Enable counter: ~(1<<0)
...
// 4. Configure Timer Output mode as PWM
	...
	if(chN == 1){
		...
		TIMx->CCER &= ~TIM_CCER_CC2P;                  			// select output polarity: active high	(set 0: active high)
		...
		TIMx->CCMR2 |= TIM_CCMR2_OC4M_1 | TIM_CCMR2_OC4M_2; // OC1M = 110 for PWM Mode 1 output on ch3
		...
		TIMx->CCER  |= TIM_CCER_CC4E;												// Enable output for ch3
	}	
...
// 2. Check System CLK: PLL or HSI
	if((RCC->CFGR & RCC_CFGR_SW_PLL) == RCC_CFGR_SW_PLL)  		fsys = 84;  // for msec 84MHz/1e+06 [usec]
